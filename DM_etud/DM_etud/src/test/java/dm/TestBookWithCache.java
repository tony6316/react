...
public class TestBookWithCache {
    ...
    public void testUpdateCacheManagement() throws BookMapperException {
        ...
        // if the cache is well managed then the old book instance has been replaced...
        assert(savedJPAbook != savedUpdatedJPAbook);
        // but we keep the same instance in the cache if the update
        // relates to the same instance
        savedUpdatedJPAbook.setAuthor("SCHINCARIOL");
        bm.update(savedUpdatedJPAbook);
        Book sameSavedUpdatedJPAbook = bm.find("978-1430219569");
        assert(sameSavedUpdatedJPAbook == savedUpdatedJPAbook);
    }
